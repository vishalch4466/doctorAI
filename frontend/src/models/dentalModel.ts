...
import { dentalConditions } from './conditions/dentalConditions';

export class DentalModel extends BaseModel {
  constructor() {
    super('Dental');
  }
  ...
  protected async trainModel() {
    ...
    try {
      const allSymptoms = Object.values(dentalConditions).flatMap(condition => condition.symptoms);
      const labels = Object.values(dentalConditions).flatMap((condition, index) => 
        Array(condition.symptoms.length).fill(index)
      );

      const embeddings = await this.encoder.embed(allSymptoms);
      const xs = embeddings;
      const ys = tf.oneHot(labels, Object.keys(dentalConditions).length);
      ...
      this.model = tf.sequential({
        layers: [
          ...
          tf.layers.dense({
            units: Object.keys(dentalConditions).length,
            activation: 'softmax'
          })
        ]
      });
      ...
      console.error('Error training dental model:', error);
      throw error;
    }
  }
  ...
  protected async getModelPrediction(input: string, probabilities: Float32Array): Promise<string> {
    ...
    const conditions = Object.values(dentalConditions);
